...
Questa esperienza consta di due parti. Nella prima parte ci si propone di montare i circuiti e valutarne il singolo funzionamento, successivamente procederemo a collegare i circuiti fra loro per formare il circuito di lock-in. Quest'ultimo circuito sarà usato per misurare l'attenuazione della differenza di potenziale in funzione del numero di lastrine di Mylar\footnote{Resina termoplastica di indice di rifrazione pari a 1.5750 (utile?)} (e quindi ricavare il coefficiente di assorbimento di tale materiale) interposte fra la sorgente luminosa (LED) e il fotodiodo (fototransistor?). Infine verificheremo che il sistema non è sensibile alla luce ambientale\footnote{Una delle tante sorgenti di rumore.}
\\
...
%discorso sugli errori da vedere
