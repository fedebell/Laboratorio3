...
E' noto l'andamento teorico della tensione: $V_{out}=V_0 exp(-n/n_0)$. Abbiamo eseguito un fit del tipo $\ln(V_{out})=b-\frac{n}{a}$ (così da fittare una retta in carta logaritmica). %magari scriviamo dettagli del fit
...
%QUi c'era una immagine dello schema a blocchi... ci stiamo dimenticando quelahce immagine????
...
Infine si ha un circuito mediatore che effettua la media del segnale in ingresso. %Le frequenze di taglio di entrambi sono dell'ordine delle decine di Hz, dunque per il segnale a $1\,\mbox{kHz}$ funzionano bene come integratori.
...
% MI ricordo di questa cosa in lab, ma dalle figure che abbiamo messo non sembra che il segnale S9 possa avere media negativa!
 ...
 %spiegazione rudimentale ma necessaria. Migliorare!
...
\caption{La forma d'onda S1 (dal generatore di funzioni) è in canale 2 (in basso), mentre S9 in canale 1 (in alto). A sinistra il deviatore è a 0\degree, nella figura a destra è a 90\degree. \label{osc:S9(zero)}}
\end{figure}
...
I dati inoltre sono in maggiore accordo con l'andamento previsto rispetto ai dati presi senza lock-in: infatti senza di esso si ha $\chi_{rid} ^2 = 7.6$ ,  mentre con si ha $\chi_{rid}^2 = 2.6$.%specificare quanto è 
