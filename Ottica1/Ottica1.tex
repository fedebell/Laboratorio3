...
\section{Conclusioni}
La misura della lunghezza d'onda del sodio nella parte A è in accordo con il valore teorico di 589 nm. La misura ottenuta nella parte B è inferiore a tale valore. Una possibile causa potrebbe essere dovuta ad errori nella calibrazione dello strumento (in particolare nella misura di $\alpha_0$). In effetti, le lunghezze d'onda misurate per le linee dell'idrogeno risultano inferiori a quelle attese\footnote{si consulti il sito del NIST} di circa 3 nm, il che è indizio di un errore sistematico. Il doppietto del sodio è visibile solo nel caso B, (maggiore risoluzione dell'elemento dispersivo), tuttavia la sovrastima degli errori non ci consente di avere una incertezza sui valori inferiore della separazione di 0.6 nm.
\end{document}