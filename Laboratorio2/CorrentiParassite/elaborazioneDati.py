...
t, dt, x, dx = pylab.loadtxt("AlluminioPieno.txt", unpack=True)
...
outliarx = numpy.array([])
outliart = numpy.array([])
y = numpy.array([])
u = numpy.array([])
dy = numpy.array([])
du = numpy.array([])
n = 4 #Numero di sigma fuori
for i in range(len(x)):
    if(numpy.abs((x[i]-fit_function(t[i], tau, a, omega, phi, k))/dx[i]) <= n):
        ...
        du = numpy.append(du, dt[i])
            ...
            outliarx = numpy.append(outliarx, x[i])
            outliart = numpy.append(outliart, t[i])
...
pylab.grid(color = "gray")
pylab.errorbar(t, x, dx, dt, "o", color="black") #aggiungere eventuali errori
pylab.errorbar(outliart, outliarx, 0, 0, "o", color = "red")
div = 100000
bucket = numpy.array([0.0 for i in range(div)])
retta = numpy.array([0.0 for i in range(div)])
inc = (t.max())/div 
for i in range(len(bucket)):
        bucket[i]=float(i)*inc
        retta[i] = fit_function(bucket[i], tau*1000, a, omega/1000, phi, k)
...
pylab.ylabel("Residui")
pylab.grid(color = "gray")
pylab.plot(t, fit_function(t, tau*1000, a , omega/1000, phi, k) - x)
...
print(N)
#Inserire il numero di millisecondi corrispondenti all'acquisizione
#In teoria ci dovrei mettere il numero in secondi tra una acquisizione e l'altra
#però palesemente non da il risultato voluto.
omega = numpy.array([((2*3.14)/(N*0.000050)) * i for i in range(N)])
