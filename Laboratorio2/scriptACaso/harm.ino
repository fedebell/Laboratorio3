...
viene fornita al pin digitale 7; la tensione viene campionata sull'ingresso
analogico A0; il campionamento avviene su 256 punti; l'intervallo temporale di campionamento
nominale e' aggiustabile in unita'  di 10 us (sconsigliato scendere sotto 50 us)
*/
...
void loop()
  ...
  if(!start) return // Se il flag e' start=0 non esegue le operazioni qui di seguito
    ...
    while (sinc==LOW) // ciclo di attesa iniziale per sincronizzazione, attende che sincPin vada alto
     ...
     for(i=0;i<2;i++) // Fa un ciclo di due letture per "scaricare" l'analogPin
       {
