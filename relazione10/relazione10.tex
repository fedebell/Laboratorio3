...
\caption{Corrente in ingresso alla porta Not in funzione di $V_{in}$\label{fig:viniout}}
\end{figure} 
...
Per misurare la corrente abbiamo misurato la caduta di potenziale $V_{ab}$ ai capi della resistenza $R_2$ tramite multimetro digitale. Per verificare che l'uscita fosse effettivamente nello stato basso, si è controllato $V_{out}$ tramite oscilloscopio. Abbiamo deciso di prendere la misura di corrente in corrispondenza del valore di $V_{OL}$ in precedenza. Tuttavia, in questo modo si ottiene $I_{OL}=$, che risulta più basso del valore riportato sul datasheet. Abbiamo quindi deciso di prendere una ulteriore misura, in corrispondenza del punto in cui si osservava una brusca variazione di $V_{out}$ e $V_{ab}$. Ciò avviene per $V_{out}=$ e $V_{ab}=$.
Così si ottiene una stima più vicina ai valori riportati nel datasheet. Un motivo per cui fallisce il metodo effettuare la misura a V_{OH} stimato è che nelle misure riportate nel grafico \ref{fig:vinvout} non si è riusciti a ottenere misure di V_{out} che non fossero del valore limite 0.144 V.
...
Così si ottiene I_{OH}=, valore in accordo con quanto riportato nel datasheet. La strategia di mettere V_{out} pari alla stima del valore di soglia ottenuto in questo caso funziona meglio perchè i dati del grafico \ref{fig:vinvout} coprono un intorno sufficientemente grande del punto in cui avviene la variazione brusca.
%discussione teorica andamento
Con questi valori si è dato una stima del \emph{fanout}.
Le correnti che determinano tale valore sono I_{OL}
e I_{IL}.
...
\end{figure} Successivamente abbiamo verificato il suo comportamento da generatore di onde quadre. La frequenza del segnale dipende dalla posizione del trimmer, e va dagli Hz ai 50 kHz a,. L'ampiezza dell'onda picco-picco è pari a $v_{pp}=3.16\pm 0.04$V.
\begin{figure}
\centering
...
\begin{figure}
\centering
\includegraphics[scale=0.6]{tphl.png}
