...
%TODO
Si è montato il circuito in figura \ref{circuito}. Nella scatola nera erano fissati il fotomoltiplicatore e la lampada a LED che serviva da sorgente luminosa. Durante l'esperienza, si sono montati i filtri nell'apposito supporto, avendo cura che fossero ortogonali al piano di appoggio della scatola e quindi al fascio luminoso.
...
Per ogni filtro si sono prese quindici misure di tensione ai capi del tubo fotomoltiplicatore e della relativa corrente.
...
L'incertezza associata ad ogni lunghezza d'onda riportata è stata presa come la semidispersione della banda passante.
 \begin{table}[!ht]
\begin{center}
\begin{tabular}{|c|c|c|}
\hline 
...
Per trovare la relazione fra frequenza dei fotoni incidenti e il relativo potenziale di azzeramento è necessario stimare dai dati quest'ultimo. Per fare ciò abbiamo usato due metodi.
\subsection{Metodo A}
...
Per stimare la corrente di emissione dell'anodo, si sino presi i dati in regime asintotico e si è eseguito un fit con costante.
I risultati dei fit sono riportati nelle tabelle  \ref{}

\subsection{Metodo B}
Per ogni filtro si è stimato il relativo potenziale di azzeramento $V_0$ corrispondente alla tensione per cui la corrente fotocatodica risulta compatibile con 0 entro l'incertezza $\delta I$.
 Per fare ciò si è eseguito un fit per ogni set di misure con la funzione modello
\begin{equation}
...
\end{equation} 
Come incertezza $\delta I$ si è preso l'errore sul parametro estratto da ogni fit.
SICuro????
